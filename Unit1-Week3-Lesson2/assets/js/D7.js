...
}        //chiedere perche il secondo parametro e` lungo come la parola
...
for (let i = 0; i < 10; i++) {
  dieciElementi.push(Math.floor(Math.random() * 101))
}
...
const valoriNumerici = [1, 26, 35, 49, 70]

const vnFiltrati = valoriNumerici.filter(function (valoriNumerici) { return valoriNumerici % 2 === 0 })

//valoriNumerici.filter (vn => vn % 2 === 0) 

console.log('***ESERCIZIO 3***')
console.log(vnFiltrati)

/* ESERCIZIO 4 (forEach)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/

const valoriSomma = [1, 3, 5, 9]
let addizione = 0

//const addizione = valoriSomma.forEach(function(valoriSomma){return valoriSomma += valoriSomma})
//chiedere  perche non funziona

valoriSomma.forEach(vn => { addizione += vn })
...
//chiedere bene lo svolgimento e la scrittura estesa della funzione
...
function incremento(valoriSomma, n) {
  const nValori = valoriSomma.map(val1 => val1 + n);
  return nValori;
}

//valoriSomma.map((valoriSomma, n) => valoriSomma + n, 0)

console.log('***ESERCIZIO 6***')
console.log(incremento(valoriSomma, 10))
