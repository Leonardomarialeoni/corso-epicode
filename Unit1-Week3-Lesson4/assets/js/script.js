...
// Oggi che giorno e`?
const now = new Date();
const getMonth = now.getMonth(); //variabile globale contenente il numero del mese in corso, che sara usata in piu di una funzione

// Adesso sappiamo in che mese siamo, dobbiamo stabilire questo mese quanti giorni ha;
// inoltre dobbiamo anche creare un array del mese, che conterra tanti array quanti sono i giorni del mese.
// Il numero di giorni, quindi, generera la griglia visiva e l'array
...
// Mi serve prendere il primo giorno del mese successivo al mese trovato e poi ottenere il giorno prima, che sara l'ultimo giorno del mese in corso;
// il numero dell'ultimo giorno del mese in corso corrisponderadi giorni totali - FUNZIONE NUMERO 2

const dayInThisMonth = () => {
    const getYear = now.getFullYear();
    const lastDayInTheMonth = new Date(getYear, getMonth + 1, 0);// In questo momento gli sto chiedendo, ad esempio, lo 0 marzo 2024; il giorno 0 ovviamente non esiste,
    //ed essendo il numero prima di uno, corrisponde al giorno prima, cioè, ad esempio, al 29 febbraio 2024
...
// creazione griglia con tutte le funzionalita - FUNZIONE NUMERO 3
const createDays = (numberOfDays) => {
    const calendarDiv = document.getElementById('calendar');
    // Ciclando il numero di giorni creo ed appendo i div corrispondenti al numero di giorni nel contenitore padre calendar
    for (let i = 0; i < numberOfDays; i++) {
        ...
        dayCellDiv.addEventListener('click', function () {
            ...
            if (appointments[i].length > 0) {
                showAppointments(i); //F FUNZIONE NUMERO 6
            } else {
...
// avviamo il tutto creando un eventlistener sul submit del form - FUNZIONE NUMERO 8
